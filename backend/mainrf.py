from ml_predictor import RockfallPredictor
...
# Add CORS middleware - THIS FIXES THE FRONTEND CONNECTION
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:3000",  # For Python HTTP server
        "http://127.0.0.1:3000",
...
@app.get("/predict")
def predict_risk(vibration: float, displacement: float, rainfall: float = 0, temperature: float = 25):
    """Predict rockfall risk using ML model"""
    try:
        result = predictor.predict(vibration, displacement, rainfall, temperature)
        return result
    except Exception as e:
        return {"error": str(e)}

@app.get("/predict/explain")
def explain_risk(vibration: float, displacement: float, rainfall: float = 0, temperature: float = 25):
    """Get explanation for prediction"""
    try:
        explanation = predictor.explain_prediction(vibration, displacement, rainfall, temperature)
...
if __name__ == "__main__":
    ...
    print("🤖 ML Model loaded - Try /predict?vibration=0.8&displacement=10")
